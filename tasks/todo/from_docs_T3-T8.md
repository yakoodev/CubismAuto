...
- [ ] Для `%TEMP%` включать только пути, содержащие `live2d` или `cubism` (case-insensitive).
- [ ] Для остальных roots не применять этот фильтр.
- [ ] Проверить, что TEMP-снимок быстрый и без лишнего мусора.
